...
/**
 * Servlet implementation class servletFeedPercorso
 */
@WebServlet("/servletFeedPercorso")
public class servletFeedPercorso extends HttpServlet implements Servlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public servletFeedPercorso() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(rs.getInt(1)==0){	
				...
				rs3.next();
				if(feed==0){
				double f = rs3.getDouble("feedp")-1;
				updb.inserimento("update utente set feedp="+f+ " where nomeutente='"+utentef+"'");
				}
				jj.put("contr", 5);
			}else{
				jj.put("contr",6);
